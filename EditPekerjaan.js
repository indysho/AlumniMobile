...
export default function EditPekerjaan({ navigation }) {
  const [nim, setNim] = useState('123456789'); // contoh data terisi
  const [perusahaan, setPerusahaan] = useState('PT Contoh');
  const [jabatan, setJabatan] = useState('Staff HRD');
  const [gaji, setGaji] = useState('5000000');
  const [lokasi, setLokasi] = useState('Jakarta');
  const [tanggalMulai, setTanggalMulai] = useState(new Date('2022-01-01'));
  const [tanggalSelesai, setTanggalSelesai] = useState(new Date('2024-01-01'));
    ...
    >
      <View style={styles.container}>
        <View style={styles.card}>
          <Text style={styles.title}>Edit Pekerjaan</Text>

          <Text style={styles.label}>Nim</Text>
          <TextInput style={styles.input} value={nim} onChangeText={setNim} />

          <Text style={styles.label}>Nama Perusahaan</Text>
          <TextInput style={styles.input} value={perusahaan} onChangeText={setPerusahaan} />

          <Text style={styles.label}>Jabatan</Text>
          <TextInput style={styles.input} value={jabatan} onChangeText={setJabatan} />
